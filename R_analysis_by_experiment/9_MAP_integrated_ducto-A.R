...
pancreas_integrated_ductsAndAcinar <- subset(pancreas.integrated, idents = c("0", "1", "3", "4", "6", "10"))
seurat_object_integrated <- pancreas_integrated_ductsAndAcinar
...
plot_cell_trajectory(seurat_object_integrated, color_by = "seurat_clusters", 
                     cell_size = 0, 
                     theta = -10,
                     show_tree = FALSE,
                     markers = "GP2",
...
pancreas.integrated_filtered <- seurat_object_integrated
my_genes <- row.names(subset(fData(pancreas.integrated_filtered),
                             gene_short_name %in% c("INS")))
